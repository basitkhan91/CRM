...
export class WorkOrderAddComponent implements OnInit {
    ...
    addWorkOrder(): void {
        ...
        this.workOrderService.add(this.workOrder).subscribe(
            result => {
                ...
                this.alertService.showMessage(this.moduleName, "Work Order Added Succesfully", MessageSeverity.success);
                for (var i = 0; i < this.workOrderPartNumbers.length; i++) {
                    this.workOrderPartNumbers[i].workOrderId = this.workOrder.iD;
                    this.workOrderPartNumberService.add(this.workOrderPartNumbers[i]).subscribe(
                        result => {
                            debugger;
                            console.log(result);
                        },
                ...
                this.alertService.showMessage(this.moduleName, "Work Order Successfully Added : " + this.workOrder.workOrderNum, MessageSeverity.success);
            },
            error => {
                debugger;
                var message = '';
    ...
    changeWorkOrderType(event): void {
        this.workOrder.workOrderTypeId = Number.parseInt(event.target.value.split('_')[1]);
    }

...
export class EditPoComponent implements OnInit {
  ...
  svalueChange(
    data //Site Valu Selection in Form
  ) {
    data.allWareHouses = [];
    data.allLocations = [];
    data.allShelfs = [];
  ...
  wareHouseValueChange(data) {
    ...
    data.binId = 0;
    this.binservice
      .getLocationDate(data.warehouseId)
      .subscribe(locationdata => {
        data.allLocations = locationdata;
      });
  }
  locationValueChange(data) {
    ...
    data.shelfId = 0;
    data.binId = 0;
    this.binservice.getShelfDate(data.locationId).subscribe(shelfdata => {
      data.allShelfs = shelfdata;
    });
  }
  shelfValueChange(data) {
    data.allBins = [];
    data.binId = 0;
    this.binservice.getBinDataById(data.shelfId).subscribe(bindata => {
      data.allBins = bindata;
    });
  }

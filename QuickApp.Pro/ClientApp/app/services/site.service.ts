...
export class SiteService {
	...
	getSiteList()
	{
		return Observable.forkJoin(
			this.siteEndpoint.getSiteEndpoint<Site[]>());
	}
	...
	historySite(siteId: number) {
		return Observable.forkJoin(this.siteEndpoint.getHistorySiteEndpoint<AuditHistory[]>(siteId));
	}
	...
	updateManagementSite(data: any) //Update Management SIte
	{
		for (let i = 0; i <= data.length; i++)
		{
			
			return this.siteEndpoint.getUpdateManagementSiteEndpoint(data, data[i].data.siteId);
		}

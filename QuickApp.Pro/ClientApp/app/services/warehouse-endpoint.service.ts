...
export class WarehouseEndpoint extends EndpointFactory {
	...
	private readonly __managementSiteURL: string = "/api/Warehouse/GetManagementSite";
    private readonly _managementUrlNew: string = "/api/Warehouse/managementWareHousePost";
	...
	getManagementSiteEndpoint<T>(siteID: any): Observable<T> {
		...
		return this.http.get<T>(endpointUrl, this.getRequestHeaders())
			.catch(error => {
				return this.handleError(error, () => this.getManagementSiteEndpoint(siteID));
			});
	}
	...
	getWarehouseStockSiteEndpoint<T>(siteID: any): Observable<T> {
		...
		return this.http.get<T>(endpointUrl, this.getRequestHeaders())
			.catch(error => {
				return this.handleError(error, () => this.getWarehouseStockSiteEndpoint(siteID));
			});
	}
    ...
    getNewWarehouseEndpoint<T>(userObject: any): Observable<T>
    {
		return this.http.post<T>(this._actionsUrlNew, JSON.stringify(userObject), this.getRequestHeaders())
			.catch(error => {
				return this.handleError(error, () => this.getNewWarehouseEndpoint(userObject));
			});
    }

    getnewManagementWareHouseData<T>(userObject: any): Observable<T> {

        return this.http.post<T>(this._managementUrlNew, JSON.stringify(userObject), this.getRequestHeaders())
            .catch(error => {
                return this.handleError(error, () => this.getnewManagementWareHouseData(userObject));
            });
    }
	...
	getManagementWarehouseEditEndpoint<T>(warehouseId: number): Observable<T> {
		...
		return this.http.get<T>(endpointUrl, this.getRequestHeaders())
			.catch(error => {
				return this.handleError(error, () => this.getManagementWarehouseEditEndpoint(warehouseId));
			});
	}
	...
	getHistoryWarehouseEndpoint<T>(warehouseId: number): Observable<T> {
		...
		return this.http.get<T>(endpointUrl, this.getRequestHeaders())
			.catch(error => {
				return this.handleError(error, () => this.getHistoryWarehouseEndpoint(warehouseId));
			});
	}

	getEditWarehouseEndpoint<T>(warehouseId?: number): Observable<T> {
		let endpointUrl = warehouseId ? `${this._actionsUrlNew}/${warehouseId}` : this._actionsUrlNew;

		return this.http.get<T>(endpointUrl, this.getRequestHeaders())
			.catch(error => {
				return this.handleError(error, () => this.getEditWarehouseEndpoint(warehouseId));
			});
	}

	getUpdateWarehouseEndpoint<T>(roleObject: any, warehouseId: number): Observable<T> {
		let endpointUrl = `${this._actionsUrlNew}/${warehouseId}`;

		return this.http.put<T>(endpointUrl, JSON.stringify(roleObject), this.getRequestHeaders())
			.catch(error => {
				return this.handleError(error, () => this.getUpdateWarehouseEndpoint(roleObject, warehouseId));
			});
	}
	...
	getDeleteWarehouseEndpoint<T>(warehouseId: number): Observable<T> {
		...
		return this.http.delete<T>(endpointUrl, this.getRequestHeaders())
			.catch(error => {
				return this.handleError(error, () => this.getDeleteWarehouseEndpoint(warehouseId));
			});
	}
	...
	getDeleteManagementWarehouseEndpoint<T>(warehouseId: number): Observable<T> {
		...
		return this.http.delete<T>(endpointUrl, this.getRequestHeaders())
			.catch(error => {
				return this.handleError(error, () => this.getDeleteManagementWarehouseEndpoint(warehouseId));
			});
	}
